...
interface DesignConfiguratorPage {
	configId: string
	imageUrl: string
		...
		<div className='relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20'>
			<div ref={containerRef} className='relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2'>
				<div className='relative bg-opacity-50 w-60 pointer-events-none aspect-[896/1831]'>
					...
					<div
						className={cn(
							'absolute inset-0 left-[3px] top-px right-[3px] rounded-[32px] bottom-px',
							`bg-${options.color.tw}`
						)}
					/>
				</div>
										...
										{COLORS.map(color => (
											<RadioGroup.Option
												...
												className={({ active, checked }) =>
													cn(
														'relative -m-0.5 flex cursive-pointer items-center justify-center rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent',
				...
				<div className='w-full px-8 h-16 bg-white'>
					...
					<div className='w-full h-full flex justify-end items-center'>
						<div className='w-full flex gap-6 items-center'>
							...
							<Button onClick={()=>saveConfig({configId, 
								color: options.color.value,
								finish: options.finish.value,
								material: options.material.value,
								model: options.model.value
							})} size='sm' className='w-full' isLoading = {isPending} disabled={isPending}>
								Continue
